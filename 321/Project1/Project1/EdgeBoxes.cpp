...
void EdgeBoxes::generate(Boxes &boxes, CellArray &E, CellArray &O, arrayf &V) {
  clusterEdges(E, O, V);
  cv::Mat eMap = cv::Mat::zeros(E.rows, E.cols, CV_8UC1);
  for (int k = 0; k < 1; ++k) {
	  for (int c = 0; c < w; ++c) {
		  for (int r = 0; r < h; ++r) {
			  ((uint8_t*)eMap.data)[r * w * 1 + c * 1 + k] = E.data[k * w * h + c * h + r];
		  }
	  }
  }
  cv::imshow("dd", eMap);
...
void EdgeBoxes::clusterEdges(CellArray &E, CellArray &O, arrayf &V) {
  ...
  //fclose(fp);
 //---------------------------------------------------------
  cv::Mat eds = cv::Mat::zeros(E.rows, E.cols, CV_8UC3);
  eds = ~eds;
  //---------------------------------------------------------
  ...
  for (c = 1; c < w - 1; ++c)
	...
	std::vector<cv::Point> pp; int nn = 0;
	//--------------------------------------------------------------------
    for (float sumv = 0; sumv < _edgeMergeThr; ) {
      ...
      for (cd = -1; cd <= 1; ++cd) for (rd = -1; rd <= 1; ++rd) {
        ...
        if (v > .5) v = 1 - v;
		ClusterNode bb;
		bb.c = cc, bb.r = rr, bb.s = v;
        vs.push(bb);           ////¸Ä
        vis.at(cc, rr) = vis_cnt;
		pp.push_back(cv::Point(bb.c, bb.r)); nn++;
      }
	...
	if (pp.size() > 9)
	{
		for (size_t x = 0; x < pp.size(); x++)
		{
			cv::circle(eds, cv::Point(pp[x].x, pp[x].y), 0.5, cv::Scalar(s0, s1, s2), 1, 8, 0);
		}
	}
    _segMag.push_back(MagSum); ++_segCnt;
  }
	cv::imshow("dddd", eds);
	//cv::imwrite("1.png", eds);
...
void EdgeBoxes::prepDataStructs(CellArray &E) {
  ...
  arrayf E1(h, w);
  for (i = 0; i < _segCnt; ++i) if (_segMag[i] > 0) {
    E1.at(_segP[i].x, _segP[i].y) = _segMag[i];
  }
...
void EdgeBoxes::scoreBox(Box &box) {
  ...
  for (i = 0; i < n; ++i) {
    ...
    for (k = 0; k < int(_segAff[j].size()); ++k) {
      ...
      } else if (_segP[q].x >= c0 && _segP[q].x <= c1 && _segP[q].y >= r0 && _segP[q].y <= r1) {
  ...
  for (i = 0; i < n; ++i) {
    k = sIds[i];
    if (_segP[k].x >= c0 && _segP[k].x <= c1 && _segP[k].y >= r0 && _segP[k].y <= r1) {
...
void EdgeBoxes::drawBox(Box &box, CellArray &E, arrayf &V) {
  ...
  for( c=0; c<w; c++ ) for( r=0; r<h; r++ ) {
    ...
    o = (_sDone.data[i]==sId) ? _sWts.data[_sMap.data[i]] :
      (_segP[i].x>=c0 && _segP[i].x<=c1 && _segP[i].y>=r0 && _segP[i].y<=r1 ) ? 0 : 1;
